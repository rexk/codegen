...
module.exports.resolvePath = function (type) {
  ...
  type = type.replace('_', '-');
  if (type === 'wrapper') {
    return './out/stackexchange/source/' + type + '.html';
  } else {
    return './out/stackexchange/source/types/' + type + '.html';
  }
};
...
module.exports.getTypeObject = function (type, callback) {
  async.auto({
    prep: function (cb) {
      ...
      if (!fs.existsSync('./out/stackexchange/source')) {
       fs.mkdirSync('./out/stackexchange/source');
      }
      if (!fs.existsSync('./out/stackexchange/source/types')) {
       fs.mkdirSync('./out/stackexchange/source/types');
      }
 
      cb(null);
    },
    cache: ['prep', function (cb) {
